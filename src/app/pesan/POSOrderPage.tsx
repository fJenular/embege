...
import { 
  ...
  HelpCircle, PhoneCall, Filter, Sparkles, X, ChevronRight
} from 'lucide-react';
...
export default function POSOrderPage() {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-[#0f172a] font-sans antialiased flex flex-col lg:flex-row">
      ...
      {/* LEFT SIDEBAR */}
      <aside className="w-full lg:w-72 bg-white border-r border-[#e2e8f0] flex flex-col p-6 shrink-0 z-30 h-screen">
        <div className="flex flex-col h-full">
          <div>
            ...
            <nav className="space-y-2">
              ...
              <button className="w-full flex items-center gap-3.5 rounded-xl px-4 py-3 text-sm font-bold sidebar-item-active text-white transition-all">
                <span className="text-base">🍽️</span>
                Pesan Menu
              </button>
              <button className="w-full flex items-center gap-3.5 rounded-xl px-4 py-3 text-sm font-bold text-muted-foreground hover:text-[#0f172a] hover:bg-slate-50 transition-all">
          ...
          {/* Footer Login Button - stays at bottom */}
          <div className="mt-auto pt-6 border-t border-slate-100">
            <div className="mb-4 rounded-xl bg-slate-50 p-3 border border-slate-100 text-center">
              <p className="text-[11px] text-muted-foreground font-medium">Khusus Staf & Admin</p>
            </div>
            <button
      ...
      <main className="flex-1 flex flex-col overflow-auto min-w-0 pb-[360px] lg:pb-0">
        ...
        <header className="bg-white border-b border-[#e2e8f0] px-6 py-4.5 flex flex-col md:flex-row items-center justify-between gap-4 z-20">
          <div className="w-full md:w-auto">
            <h2 className="text-2xl font-black text-slate-900 tracking-tight">Katering Premium EMBEGE</h2>
            <p className="text-xs text-muted-foreground mt-0.5">Pilih menu catering terbaik untuk kebutuhan acara Anda.</p>
          </div>
      ...
      <aside className="fixed bottom-0 left-0 right-0 lg:static lg:w-[420px] bg-white border-t border-[#e2e8f0] lg:border-l flex flex-col shrink-0 z-30 h-[360px] lg:h-auto">
      ...
      {showSuccessModal && lastInvoice && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[2rem] w-full max-w-lg p-8 shadow-2xl relative overflow-hidden animate-scale-up border border-slate-100">
            ...
            <button 
              ...
              className="absolute top-6 right-6 text-slate-400 hover:text-slate-600 hover:bg-slate-50 p-2 rounded-xl"
            >
              <X className="w-5 h-5" />
            </button>
